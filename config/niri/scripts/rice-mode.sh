...
kitty --class=kitty-fastfetch -o background_opacity=0.55 sh -c "sleep 1;fastfetch --logo-type file --logo ~/.config/fastfetch/templates/arch.txt; exec fish" &
...
BOTTOM_X=$((MONITOR_WIDTH - 720 - 14))
BOTTOM_Y=$((MONITOR_HEIGHT - 400 - 14))
# Настроить все окна через batch команды (быстрее)
hyprctl --batch "\
dispatch focuswindow class:kitty-pipes; \
dispatch togglefloating; \
dispatch resizeactive exact 720 400; \
dispatch moveactive exact 63 14; \
